...
class ServerWebsocket(WebSocketClient):
    ...
    def received_message(self, m):
        logger.debug(
            "%s: Got message from server of type %s" % (self.request_id, str(type(m)))
        )
        if self.state == self.__class__.STATE_CONNECTED:
            ...
            if (
                ...
                logger.info(
                    "%s: Ignoring EOS, worker already in state %d"
                ...
                if isinstance(m, ws4py.messaging.BinaryMessage):
                    ...
                    if "adaptation_state" in props:
                        ...
                        if as_props.get("type", "") == "string+gzip+base64":
                            ...
                            logger.warning(
                                "%s: Cannot handle adaptation state type "
                                % (self.request_id, as_props.get("type", ""))
                            )
                    else:
                        logger.warning(
                            "%s: Got JSON message but don't know what to do with it"
                            % (self.request_id)
                        )
            else:
                logger.info(
                    "%s: Ignoring data, worker already in state %d"
                    % (self.request_id, self.state)
                )
        ...
        if self.state != self.STATE_FINISHED:
            ...
            while self.state == self.STATE_CANCELLING:
                ...
                if counter > 30:
                    ...
                    logger.info(
                        "%s: Giving up waiting after %d tries"
    ...
    def _on_result(self, result, final):
        try:
            ...
            logger.info(
                "%s: Postprocessing (final=%s) result.." % (self.request_id, final)
            )
            # processed_transcripts = yield self.post_process([result], blocking=False)
            partial_result = json.loads(result)
            partial_result["segment"] = self.num_segments
            partial_result["id"] = self.request_id
            if partial_result.get("status", -1) == common.STATUS_SUCCESS:
                logger.debug(
                    u"%s: Before postprocessing: %s"
                    % (self.request_id, repr(partial_result).decode("unicode-escape"))
                )
                # partial_result = yield self.post_process_full(partial_result)
                logger.info("%s: Partial Postprocessing done." % self.request_id)
                logger.debug(
                    u"%s: After partial postprocessing: %s"
                    % (self.request_id, repr(partial_result).decode("unicode-escape"))
                )

                try:
                    self.send(json.dumps(partial_result))
                except:
                    e = sys.exc_info()[1]
    ...
    def _on_full_result(self, full_result_json):
        try:
            ...
            if full_result.get("status", -1) == common.STATUS_SUCCESS:
                ...
                logger.info(
                    "%s: Result status is %d, forwarding the result to the server anyway"
        ...
        try:
            ...
            if word != "<#s>":
                ...
                processed_transcript = (
                    yield self.post_process([self.partial_transcript], blocking=False)
                )[0]
                if processed_transcript:
                    ...
                    event = dict(
                        ...
                        result=dict(
                            hypotheses=[dict(transcript=processed_transcript)],
                            final=False,
                        ),
                    )
                    self.send(json.dumps(event))
                ...
                processed_transcript = (
                    yield self.post_process(self.partial_transcript, blocking=True)
                )
                logger.info("%s: Postprocessing done." % self.request_id)
                event = dict(
                    status=common.STATUS_SUCCESS,
                    segment=self.num_segments,
                    result=dict(
                        hypotheses=[dict(transcript=processed_transcript)], final=True
                    ),
                )
                self.send(json.dumps(event))
                self.partial_transcript = ""
    ...
    def _on_eos(self, data=None):
        ...
        while self.num_processing_threads > 0:
            logging.debug(
                "Waiting until processing threads finish (%d)"
    ...
    def send_adaptation_state(self):
        if hasattr(self.decoder_pipeline, "get_adaptation_state"):
            ...
            logger.info(
                "%s: Adaptation state not supported by the decoder, not sending it."
                % (self.request_id)
            )
            ...
            try:
                with (yield self.post_processor_lock.acquire(timeout)):
                    ...
                    for text in texts:
                        ...
                        logging.debug(
                            "%s: Starting postprocessing: %s" % (self.request_id, text)
                        )
                        text = yield self.post_processor.stdout.read_until("\n")
                        text = text.decode("utf-8")
                        logging.debug(
                            "%s: Postprocessing returned: %s" % (self.request_id, text)
                        )
                        text = text.strip()
                        text = text.replace("\\n", "\n")
                        result.append(text)
                ...
                logging.debug(
                    "%s: Skipping postprocessing since post-processor already in use"
                    % (self.request_id)
                )
    ...
    def post_process_full(self, full_result):
        if self.full_post_processor:
            ...
            for (i, hyp) in enumerate(
                full_result.get("result", {}).get("hypotheses", [])
            ):
...
def main():
    ...
    parser.add_argument(
        "-c", "--conf", dest="conf", help="YAML file with decoder configuration"
    )
